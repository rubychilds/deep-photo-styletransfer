...
def bipartite_matching(varargin):
...
def bipartite_matching_setup(A, ei, ej, n, m):
    ...
    ci = np.zeros(nedges+n, 1)
    ai = np.zeros(nedges+n, 1)
    if triplet:
        tripi = np.zeros(nedges+n, 1) # triplet index
    else:
        tripi = []
    ...
    for i=1:nedges
        if triplet:
            tripi(rp(nzi(i))+1) = i # triplet index
        ai(rp(nzi(i))+1) = nzv(i)
        ci(rp(nzi(i))+1) = nzj(i)
        rp(nzi(i)) = rp(nzi(i))+1
