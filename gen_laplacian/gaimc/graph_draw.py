def graph_draw(adj, xy, varargin):
...
def textoval(x, y, str, fontsize, c):
...
def my_arrow(varargin):
  ...
  while (any(axnotdone))
    ...
    set(curax,'Position',axposoldunits)
  	...
  	if not arrow_WarpToFill(notstretched,manualcamera,curax):
  ...
  ii = find(isnan(start(:)))
  if not isempty(ii):
      start(ii) = axm(ii)+axr(ii)/2
  ii = find(isnan(stop(:)))
  if not isempty(ii):
      stop(ii) = axm(ii)+axr(ii)/2
  ii = find(isnan(crossdir(:)))
  if not isempty(ii):
      crossdir(ii) = zeros(length(ii),1)
  ii = find(isnan(len))
  if not isempty(ii):
      len(ii) = ones(length(ii),1)*deflen

  baseangle(ii) = ones(length(ii),1)*defbaseangle
  tipangle(ii) = ones(length(ii),1)*deftipangle
  wid(ii) = ones(length(ii),1) * defwid
  ends(ii) = ones(length(ii),1) * defends
  ...
  if (any(imag(crossdir(:))not =0)):
      ii = find(any(imag(crossdir)not =0))
  	crossdir(:,ii) = cross((stop(:,ii)-start(:,ii))./axr(:,ii), imag(crossdir(:,ii))).*axr(:,ii)
  ...
  if not isempty(ii):
	...
	if (oneax):
		...
		tmp2=np.zeros(4,16*numii); tmp2(:)=tmp1(:)
		X0=np/zeros(4,4*numii); X0(:)=sum(tmp2)'
  	...
  	#	inverse transform the cross points
  		if (oneax):
            ...
            tmp1=[Xp;Xp;Xp;Xp]
            tmp1=invT(:,[ii ii ii ii]).*tmp1
  		    tmp2=zeros(4,16*numii)
            tmp2(:) = tmp1(:)
  		    Xp=zeros(4,4*numii)
            Xp(:) = sum(tmp2)'
  ...
  #	compute start points
  	...
  	X0=X0./(ones(4,1)*X0(4,:))
  #	compute stop points
  	tmp1 = ([start tipcross basecross sbasecross stipcross stop stipcross sbasecross basecross tipcross start] - axm11) ./ axr11
  	tmp1 = [tmp1; ones(1,size(tmp1,2))]
  	if (oneax):
        Xf=T*tmp1
...
def arrow_WarpToFill(notstretched,manualcamera,curax): ##ok<INUSL>
  # check if we are in "WarpToFill" mode.
  	...
  	# out = not ( any(notstretched) & any(manualcamera) );
    return out
